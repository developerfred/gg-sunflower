...
export class LandCreated extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save LandCreated entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type LandCreated must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("LandCreated", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): LandCreated | null {
    return changetype<LandCreated | null>(
      store.get_in_block("LandCreated", id.toHexString())
    );
  }

  static load(id: Bytes): LandCreated | null {
    return changetype<LandCreated | null>(
      store.get("LandCreated", id.toHexString())
    );
  }
  ...
  set landAddress(value: Bytes) {
    this.set("landAddress", Value.fromBytes(value));
  }
